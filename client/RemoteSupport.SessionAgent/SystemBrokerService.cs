...
/// <summary>
/// Installs a temporary demand-start LocalSystem service that launches the
/// session helper with a SYSTEM token whose session id is switched to the
/// agent's session. A System-integrity helper passes every UIPI boundary for
/// SendInput without UIAccess or code signing. The service and its files are
/// removed when the agent exits.
/// </summary>
internal sealed class SystemBrokerService : IDisposable
{
    ...
    private SystemBrokerService(SafeServiceHandle manager, SafeServiceHandle service, string directory)
    {
        _manager = manager;
        _service = service;
    ...
    public static SystemBrokerService? TryStart()
    {
        try
        {
            ...
            {
                AppDiagnostics.Write("Broker runtime is not embedded; the elevated helper path remains available.");
                return null;
            }
            ...
            try
            {
                ...
                var quotedPath = "\"" + servicePath + "\" --session " + sessionId + " --virtual " + virtualMetrics;
                ...
                AppDiagnostics.Write("Temporary SYSTEM broker service is RUNNING; the System-integrity helper will start shortly.");
                return new SystemBrokerService(manager, service, directory);
            }
            catch
            {
                manager.Dispose();
        ...
        {
            AppDiagnostics.Write("SYSTEM broker could not be started; the elevated helper path remains available.", exception);
            return null;
        }
    }
    ...
    private static void WaitUntilRunning(SafeServiceHandle service)
    {
        ...
        while (true)
        {
            ...
            if (unchecked(Environment.TickCount - deadline) >= 0)
                throw new TimeoutException("Input runtime did not reach RUNNING state within 15 seconds.");
            Thread.Sleep(100);
        }
    }
    ...
    public void Dispose()
    {
        ...
        for (var attempt = 0; attempt < 30; attempt++)
        {
            if (!QueryServiceStatus(_service, ref status) || status.CurrentState == ServiceStopped) break;
            Thread.Sleep(100);
        }
        if (!DeleteService(_service))
        {
            var error = Marshal.GetLastWin32Error();
            if (error != 1072) AppDiagnostics.Write("Temporary broker service deletion failed. Win32Error=" + error);
        }
        _service.Dispose();
        _manager.Dispose();
        TryDelete(_directory);
        AppDiagnostics.Write("Temporary SYSTEM broker service stopped.");
    }
