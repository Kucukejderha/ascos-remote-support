...
- Durum: Uçtan uca kaynak akışı ve aynı EXE içindeki geçici SYSTEM service/interactive helper ayrımı bağlıdır. Helper, Session 0 SYSTEM tokenını başka oturuma taşımak yerine yükseltilmiş istemcinin gerçek etkileşimli kullanıcı/logon tokenından üretilir. GitHub Actions Windows Server 2022 üzerinde gerçek x64 Release derlemesi, CLR yokluğu, statik CRT bağımlılık denetimi ve 10 MB boyut kapısı `9021e3ab1841b70d6a27ff96366ab91ffdd8e82b` commitinde başarılıdır. Hedef işletim sistemi VM matrisi tamamlanmadığı için kararlı müşteri sürümü olarak yayınlanmaz. Input ACK yalnız helper IPC sonucu ile `SendInput` beklenen olay sayısını gerçekten döndürürse başarılıdır.
