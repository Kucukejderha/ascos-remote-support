...
Alpha.17 uses a persistent `%ProgramFiles%\RotaLink\Runtime\1.1.0-alpha.17` runtime. The service launches SessionHelper as LocalSystem in the RotaLink client's actual session (including RDP sessions), so the helper no longer requests UIAccess. The input desktop is opened with the complete desktop access mask including `DESKTOP_JOURNALPLAYBACK`. The helper accepts IPC only from the RotaLink client process ID passed through the service. Production builds still require trusted Authenticode signatures and RotaLink validates each extracted executable with `WinVerifyTrust` before registering or starting the service.
